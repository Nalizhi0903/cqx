...
class UserInfo 
{
  ...
  public:
    UserInfo()
    {}
    UserInfo(const string& username, const string& sex, const string& phonenumber, const string& passwd, int userid):_username(username), _sex(sex), _phonenumber(phonenumber), _passwd(passwd), _userid(userid), _user_status(OFFLINE), _tcp_socket(-1), _subscribe_list(10)
    {}
    ~UserInfo()
    {}
  public:
...
{
  public:
    CustomerManager():_user_map(0),  _dbs(nullptr)
    {}
    ~CustomerManager()
    {
      if(_dbs != nullptr)
